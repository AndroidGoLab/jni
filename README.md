...
All 53 examples are self-contained NativeActivity apps — zero Java code.
Each lives in `examples/<name>/` with its own `Makefile`:
...
4. **gRPC Layer** — See [jni-proxy](https://github.com/AndroidGoLab/jni-proxy) for gRPC remote access (proto/grpc generation lives in that repo).
