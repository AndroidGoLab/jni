// Code generated by javagen. DO NOT EDIT.

package devicelock

import (
	"fmt"
	"sync"
	"unsafe"

	"github.com/AndroidGoLab/jni"
)

// Ensure imports are used.
var (
	_ = fmt.Errorf
	_ = unsafe.Pointer(nil)
)

// jniTrue is the JNI representation of a true boolean value.
const jniTrue uint8 = 1

var (
	initOnce sync.Once
	initErr  error

	clsDeviceLockManager         *jni.GlobalRef
	midDeviceLockManagerToString jni.MethodID

	clsDeviceId         *jni.GlobalRef
	midDeviceIdGetId    jni.MethodID
	midDeviceIdGetType  jni.MethodID
	midDeviceIdToString jni.MethodID
)

func ensureInit(env *jni.Env) error {
	initOnce.Do(func() {
		initErr = doInit(env)
	})
	return initErr
}

// Init resolves all JNI class and method references for this package.
// It is safe to call multiple times (uses sync.Once internally).
// Exported for E2E testing on Android.
func Init(env *jni.Env) error {
	return ensureInit(env)
}

func doInit(env *jni.Env) error {
	var c *jni.Class
	var err error

	c, err = env.FindClass("android/devicelock/DeviceLockManager")
	if err != nil {
		// Class may not exist on this device's API level; skip and
		// report at invocation time instead of failing the entire init.
		env.ExceptionClear()
	} else {
		clsDeviceLockManager = env.NewGlobalRef(&c.Object)

		midDeviceLockManagerToString, err = env.GetMethodID((*jni.Class)(unsafe.Pointer(clsDeviceLockManager)), "toString", "()Ljava/lang/String;")
		if err != nil {
			// Method may not exist on this device's API level; skip and
			// report at invocation time instead of failing the entire init.
			env.ExceptionClear()
		}

	}

	c, err = env.FindClass("android/devicelock/DeviceId")
	if err != nil {
		// Class may not exist on this device's API level; skip and
		// report at invocation time instead of failing the entire init.
		env.ExceptionClear()
	} else {
		clsDeviceId = env.NewGlobalRef(&c.Object)

		midDeviceIdGetId, err = env.GetMethodID((*jni.Class)(unsafe.Pointer(clsDeviceId)), "getId", "()Ljava/lang/String;")
		if err != nil {
			// Method may not exist on this device's API level; skip and
			// report at invocation time instead of failing the entire init.
			env.ExceptionClear()
		}

		midDeviceIdGetType, err = env.GetMethodID((*jni.Class)(unsafe.Pointer(clsDeviceId)), "getType", "()I")
		if err != nil {
			// Method may not exist on this device's API level; skip and
			// report at invocation time instead of failing the entire init.
			env.ExceptionClear()
		}

		midDeviceIdToString, err = env.GetMethodID((*jni.Class)(unsafe.Pointer(clsDeviceId)), "toString", "()Ljava/lang/String;")
		if err != nil {
			// Method may not exist on this device's API level; skip and
			// report at invocation time instead of failing the entire init.
			env.ExceptionClear()
		}

	}

	return nil
}
