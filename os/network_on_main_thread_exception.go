// Code generated by javagen. DO NOT EDIT.

package os

import (
	"fmt"
	"unsafe"

	"github.com/AndroidGoLab/jni"
	"github.com/AndroidGoLab/jni/app"
)

// Ensure imports are used.
var (
	_ = fmt.Errorf
	_ = unsafe.Pointer(nil)
	_ *app.Context
)

// NetworkOnMainThreadException wraps android.os.NetworkOnMainThreadException.
type NetworkOnMainThreadException struct {
	VM  *jni.VM
	Obj *jni.GlobalRef
}

// NewNetworkOnMainThreadException creates a new android.os.NetworkOnMainThreadException instance.
func NewNetworkOnMainThreadException(vm *jni.VM) (*NetworkOnMainThreadException, error) {
	var t NetworkOnMainThreadException
	t.VM = vm

	err := vm.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			return err
		}
		obj, err := env.NewObject((*jni.Class)(unsafe.Pointer(clsNetworkOnMainThreadException)), midNetworkOnMainThreadExceptionCtor)
		if err != nil {
			return err
		}
		t.Obj = env.NewGlobalRef(obj)
		return nil
	})
	if err != nil {
		return nil, err
	}
	return &t, nil
}

// ToString calls android.os.NetworkOnMainThreadException.toString.
func (m *NetworkOnMainThreadException) ToString() (string, error) {
	var result string
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midNetworkOnMainThreadExceptionToString == nil {
			callErr = fmt.Errorf("android.os.NetworkOnMainThreadException.toString is not available on this device")
			return callErr
		}
		var resultObj *jni.Object
		resultObj, callErr = env.CallObjectMethod(
			m.Obj,
			midNetworkOnMainThreadExceptionToString,
		)
		if callErr != nil {
			return callErr
		}
		result = env.GoString((*jni.String)(unsafe.Pointer(resultObj)))
		return callErr
	})
	return result, callErr
}
