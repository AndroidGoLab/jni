// Code generated by javagen. DO NOT EDIT.

package p2p

import (
	"fmt"
	"unsafe"

	"github.com/AndroidGoLab/jni"
	"github.com/AndroidGoLab/jni/app"
)

// Ensure imports are used.
var (
	_ = fmt.Errorf
	_ = unsafe.Pointer(nil)
	_ *app.Context
)

// WifiP2pManagerChannelListener wraps android.net.wifi.p2p.WifiP2pManager$ChannelListener.
type WifiP2pManagerChannelListener struct {
	VM  *jni.VM
	Obj *jni.GlobalRef
}

// OnChannelDisconnected calls android.net.wifi.p2p.WifiP2pManager$ChannelListener.onChannelDisconnected.
func (m *WifiP2pManagerChannelListener) OnChannelDisconnected() error {

	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midWifiP2pManagerChannelListenerOnChannelDisconnected == nil {
			callErr = fmt.Errorf("android.net.wifi.p2p.WifiP2pManager$ChannelListener.onChannelDisconnected is not available on this device")
			return callErr
		}
		callErr = env.CallVoidMethod(
			m.Obj,
			midWifiP2pManagerChannelListenerOnChannelDisconnected,
		)
		return callErr
	})
	return callErr
}

// ToString calls android.net.wifi.p2p.WifiP2pManager$ChannelListener.toString.
func (m *WifiP2pManagerChannelListener) ToString() (string, error) {
	var result string
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midWifiP2pManagerChannelListenerToString == nil {
			callErr = fmt.Errorf("android.net.wifi.p2p.WifiP2pManager$ChannelListener.toString is not available on this device")
			return callErr
		}
		var resultObj *jni.Object
		resultObj, callErr = env.CallObjectMethod(
			m.Obj,
			midWifiP2pManagerChannelListenerToString,
		)
		if callErr != nil {
			return callErr
		}
		result = env.GoString((*jni.String)(unsafe.Pointer(resultObj)))
		return callErr
	})
	return result, callErr
}
