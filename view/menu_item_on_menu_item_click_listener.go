// Code generated by javagen. DO NOT EDIT.

package view

import (
	"fmt"
	"unsafe"

	"github.com/AndroidGoLab/jni"
	"github.com/AndroidGoLab/jni/app"
)

// Ensure imports are used.
var (
	_ = fmt.Errorf
	_ = unsafe.Pointer(nil)
	_ *app.Context
)

// MenuItemOnMenuItemClickListener wraps android.view.MenuItem$OnMenuItemClickListener.
type MenuItemOnMenuItemClickListener struct {
	VM  *jni.VM
	Obj *jni.GlobalRef
}

// OnMenuItemClick calls android.view.MenuItem$OnMenuItemClickListener.onMenuItemClick.
func (m *MenuItemOnMenuItemClickListener) OnMenuItemClick(arg0 *jni.Object) (bool, error) {
	var result bool
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midMenuItemOnMenuItemClickListenerOnMenuItemClick == nil {
			callErr = fmt.Errorf("android.view.MenuItem$OnMenuItemClickListener.onMenuItemClick is not available on this device")
			return callErr
		}

		var resultRaw uint8
		resultRaw, callErr = env.CallBooleanMethod(
			m.Obj,
			midMenuItemOnMenuItemClickListenerOnMenuItemClick, jni.ObjectValue(arg0),
		)
		if callErr != nil {
			return callErr
		}
		result = resultRaw != 0
		return callErr
	})
	return result, callErr
}

// ToString calls android.view.MenuItem$OnMenuItemClickListener.toString.
func (m *MenuItemOnMenuItemClickListener) ToString() (string, error) {
	var result string
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midMenuItemOnMenuItemClickListenerToString == nil {
			callErr = fmt.Errorf("android.view.MenuItem$OnMenuItemClickListener.toString is not available on this device")
			return callErr
		}
		var resultObj *jni.Object
		resultObj, callErr = env.CallObjectMethod(
			m.Obj,
			midMenuItemOnMenuItemClickListenerToString,
		)
		if callErr != nil {
			return callErr
		}
		result = env.GoString((*jni.String)(unsafe.Pointer(resultObj)))
		return callErr
	})
	return result, callErr
}
