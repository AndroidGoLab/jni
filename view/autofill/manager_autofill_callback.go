// Code generated by javagen. DO NOT EDIT.

package autofill

import (
	"fmt"
	"unsafe"

	"github.com/AndroidGoLab/jni"
	"github.com/AndroidGoLab/jni/app"
)

// Ensure imports are used.
var (
	_ = fmt.Errorf
	_ = unsafe.Pointer(nil)
	_ *app.Context
)

// ManagerAutofillCallback wraps android.view.autofill.AutofillManager$AutofillCallback.
type ManagerAutofillCallback struct {
	VM  *jni.VM
	Obj *jni.GlobalRef
}

// OnAutofillEvent2 calls android.view.autofill.AutofillManager$AutofillCallback.onAutofillEvent.
func (m *ManagerAutofillCallback) OnAutofillEvent2(arg0 *jni.Object, arg1 int32) error {

	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midManagerAutofillCallbackOnAutofillEvent2 == nil {
			callErr = fmt.Errorf("android.view.autofill.AutofillManager$AutofillCallback.onAutofillEvent is not available on this device")
			return callErr
		}

		callErr = env.CallVoidMethod(
			m.Obj,
			midManagerAutofillCallbackOnAutofillEvent2, jni.ObjectValue(arg0), jni.IntValue(arg1),
		)
		return callErr
	})
	return callErr
}

// OnAutofillEvent3_1 calls android.view.autofill.AutofillManager$AutofillCallback.onAutofillEvent.
func (m *ManagerAutofillCallback) OnAutofillEvent3_1(
	arg0 *jni.Object,
	arg1 int32,
	arg2 int32,
) error {

	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midManagerAutofillCallbackOnAutofillEvent3_1 == nil {
			callErr = fmt.Errorf("android.view.autofill.AutofillManager$AutofillCallback.onAutofillEvent is not available on this device")
			return callErr
		}

		callErr = env.CallVoidMethod(
			m.Obj,
			midManagerAutofillCallbackOnAutofillEvent3_1, jni.ObjectValue(arg0), jni.IntValue(arg1), jni.IntValue(arg2),
		)
		return callErr
	})
	return callErr
}

// ToString calls android.view.autofill.AutofillManager$AutofillCallback.toString.
func (m *ManagerAutofillCallback) ToString() (string, error) {
	var result string
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midManagerAutofillCallbackToString == nil {
			callErr = fmt.Errorf("android.view.autofill.AutofillManager$AutofillCallback.toString is not available on this device")
			return callErr
		}
		var resultObj *jni.Object
		resultObj, callErr = env.CallObjectMethod(
			m.Obj,
			midManagerAutofillCallbackToString,
		)
		if callErr != nil {
			return callErr
		}
		result = env.GoString((*jni.String)(unsafe.Pointer(resultObj)))
		return callErr
	})
	return result, callErr
}
