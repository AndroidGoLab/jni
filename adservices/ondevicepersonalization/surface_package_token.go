// Code generated by javagen. DO NOT EDIT.

package ondevicepersonalization

import (
	"fmt"
	"unsafe"

	"github.com/AndroidGoLab/jni"
	"github.com/AndroidGoLab/jni/app"
)

// Ensure imports are used.
var (
	_ = fmt.Errorf
	_ = unsafe.Pointer(nil)
	_ *app.Context
)

// SurfacePackageToken wraps android.adservices.ondevicepersonalization.SurfacePackageToken.
type SurfacePackageToken struct {
	VM  *jni.VM
	Obj *jni.GlobalRef
}

// ToString calls android.adservices.ondevicepersonalization.SurfacePackageToken.toString.
func (m *SurfacePackageToken) ToString() (string, error) {
	var result string
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midSurfacePackageTokenToString == nil {
			callErr = fmt.Errorf("android.adservices.ondevicepersonalization.SurfacePackageToken.toString is not available on this device")
			return callErr
		}
		var resultObj *jni.Object
		resultObj, callErr = env.CallObjectMethod(
			m.Obj,
			midSurfacePackageTokenToString,
		)
		if callErr != nil {
			return callErr
		}
		result = env.GoString((*jni.String)(unsafe.Pointer(resultObj)))
		return callErr
	})
	return result, callErr
}
