// Code generated by javagen. DO NOT EDIT.

package app

import (
	"fmt"
	"unsafe"

	"github.com/AndroidGoLab/jni"
)

// Ensure imports are used.
var (
	_ = fmt.Errorf
	_ = unsafe.Pointer(nil)
)

// PendingIntentCanceledException wraps android.app.PendingIntent$CanceledException.
type PendingIntentCanceledException struct {
	VM  *jni.VM
	Obj *jni.GlobalRef
}

// ToString calls android.app.PendingIntent$CanceledException.toString.
func (m *PendingIntentCanceledException) ToString() (string, error) {
	var result string
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midPendingIntentCanceledExceptionToString == nil {
			callErr = fmt.Errorf("android.app.PendingIntent$CanceledException.toString is not available on this device")
			return callErr
		}
		var resultObj *jni.Object
		resultObj, callErr = env.CallObjectMethod(
			m.Obj,
			midPendingIntentCanceledExceptionToString,
		)
		if callErr != nil {
			return callErr
		}
		result = env.GoString((*jni.String)(unsafe.Pointer(resultObj)))
		return callErr
	})
	return result, callErr
}
