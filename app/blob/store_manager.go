// Code generated by javagen. DO NOT EDIT.

package blob

import (
	"fmt"
	"unsafe"

	"github.com/AndroidGoLab/jni"
	"github.com/AndroidGoLab/jni/app"
)

// Ensure imports are used.
var (
	_ = fmt.Errorf
	_ = unsafe.Pointer(nil)
	_ *app.Context
)

const serviceNameStoreManager = "blob_store"

// StoreManager wraps android.app.blob.BlobStoreManager.
type StoreManager struct {
	VM  *jni.VM
	Ctx *app.Context
	Obj *jni.GlobalRef
}

// NewStoreManager obtains android.app.blob.BlobStoreManager from the Android system service manager.
func NewStoreManager(ctx *app.Context) (*StoreManager, error) {
	if ctx == nil {
		return nil, fmt.Errorf("StoreManager: nil Context")
	}
	var mgr StoreManager
	mgr.VM = ctx.VM
	mgr.Ctx = ctx

	err := mgr.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			return err
		}
		svc, err := ctx.GetSystemService(serviceNameStoreManager)
		if err != nil {
			return err
		}
		if svc == nil || svc.Ref() == 0 {
			return fmt.Errorf("%s service not available", serviceNameStoreManager)
		}
		// GetSystemService already returns a GlobalRef, so use it directly
		// instead of wrapping again (which would leak the original).
		mgr.Obj = svc
		return nil
	})
	if err != nil {
		return nil, err
	}
	return &mgr, nil
}

// Close releases the global reference to the underlying Java object.
// After Close, the StoreManager must not be used.
func (m *StoreManager) Close() {
	if m.Obj != nil {
		m.VM.Do(func(env *jni.Env) error {
			env.DeleteGlobalRef(m.Obj)
			m.Obj = nil
			return nil
		})
	}
}

// AbandonSession calls android.app.blob.BlobStoreManager.abandonSession.
func (m *StoreManager) AbandonSession(arg0 int64) error {

	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midStoreManagerAbandonSession == nil {
			callErr = fmt.Errorf("android.app.blob.BlobStoreManager.abandonSession is not available on this device")
			return callErr
		}

		callErr = env.CallVoidMethod(
			m.Obj,
			midStoreManagerAbandonSession, jni.LongValue(arg0),
		)
		return callErr
	})
	return callErr
}

// AcquireLease2 calls android.app.blob.BlobStoreManager.acquireLease.
func (m *StoreManager) AcquireLease2(arg0 *jni.Object, arg1 int32) error {

	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midStoreManagerAcquireLease2 == nil {
			callErr = fmt.Errorf("android.app.blob.BlobStoreManager.acquireLease is not available on this device")
			return callErr
		}

		callErr = env.CallVoidMethod(
			m.Obj,
			midStoreManagerAcquireLease2, jni.ObjectValue(arg0), jni.IntValue(arg1),
		)
		return callErr
	})
	return callErr
}

// AcquireLease3_1 calls android.app.blob.BlobStoreManager.acquireLease.
func (m *StoreManager) AcquireLease3_1(
	arg0 *jni.Object,
	arg1 int32,
	arg2 int64,
) error {

	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midStoreManagerAcquireLease3_1 == nil {
			callErr = fmt.Errorf("android.app.blob.BlobStoreManager.acquireLease is not available on this device")
			return callErr
		}

		callErr = env.CallVoidMethod(
			m.Obj,
			midStoreManagerAcquireLease3_1, jni.ObjectValue(arg0), jni.IntValue(arg1), jni.LongValue(arg2),
		)
		return callErr
	})
	return callErr
}

// AcquireLease2_2 calls android.app.blob.BlobStoreManager.acquireLease.
func (m *StoreManager) AcquireLease2_2(arg0 *jni.Object, arg1 string) error {

	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midStoreManagerAcquireLease2_2 == nil {
			callErr = fmt.Errorf("android.app.blob.BlobStoreManager.acquireLease is not available on this device")
			return callErr
		}

		jArg1, err := env.NewStringUTF(arg1)
		if err != nil {
			return err
		}
		defer env.DeleteLocalRef(&jArg1.Object)

		callErr = env.CallVoidMethod(
			m.Obj,
			midStoreManagerAcquireLease2_2, jni.ObjectValue(arg0), jni.ObjectValue(&jArg1.Object),
		)
		return callErr
	})
	return callErr
}

// AcquireLease3_3 calls android.app.blob.BlobStoreManager.acquireLease.
func (m *StoreManager) AcquireLease3_3(
	arg0 *jni.Object,
	arg1 string,
	arg2 int64,
) error {

	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midStoreManagerAcquireLease3_3 == nil {
			callErr = fmt.Errorf("android.app.blob.BlobStoreManager.acquireLease is not available on this device")
			return callErr
		}

		jArg1, err := env.NewStringUTF(arg1)
		if err != nil {
			return err
		}
		defer env.DeleteLocalRef(&jArg1.Object)

		callErr = env.CallVoidMethod(
			m.Obj,
			midStoreManagerAcquireLease3_3, jni.ObjectValue(arg0), jni.ObjectValue(&jArg1.Object), jni.LongValue(arg2),
		)
		return callErr
	})
	return callErr
}

// CreateSession calls android.app.blob.BlobStoreManager.createSession.
func (m *StoreManager) CreateSession(arg0 *jni.Object) (int64, error) {
	var result int64
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midStoreManagerCreateSession == nil {
			callErr = fmt.Errorf("android.app.blob.BlobStoreManager.createSession is not available on this device")
			return callErr
		}

		result, callErr = env.CallLongMethod(
			m.Obj,
			midStoreManagerCreateSession, jni.ObjectValue(arg0),
		)
		if callErr != nil {
			return callErr
		}
		return callErr
	})
	return result, callErr
}

// GetLeasedBlobs calls android.app.blob.BlobStoreManager.getLeasedBlobs.
func (m *StoreManager) GetLeasedBlobs() (*jni.Object, error) {
	var result *jni.Object
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midStoreManagerGetLeasedBlobs == nil {
			callErr = fmt.Errorf("android.app.blob.BlobStoreManager.getLeasedBlobs is not available on this device")
			return callErr
		}
		result, callErr = env.CallObjectMethod(
			m.Obj,
			midStoreManagerGetLeasedBlobs,
		)
		if callErr != nil {
			return callErr
		}
		// Convert the JNI local reference to a global reference so the
		// returned object remains valid outside this vm.Do scope.
		if result != nil {
			localRef := result
			result = env.NewGlobalRef(localRef)
			env.DeleteLocalRef(localRef)
		}
		return callErr
	})
	return result, callErr
}

// GetRemainingLeaseQuotaBytes calls android.app.blob.BlobStoreManager.getRemainingLeaseQuotaBytes.
func (m *StoreManager) GetRemainingLeaseQuotaBytes() (int64, error) {
	var result int64
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midStoreManagerGetRemainingLeaseQuotaBytes == nil {
			callErr = fmt.Errorf("android.app.blob.BlobStoreManager.getRemainingLeaseQuotaBytes is not available on this device")
			return callErr
		}
		result, callErr = env.CallLongMethod(
			m.Obj,
			midStoreManagerGetRemainingLeaseQuotaBytes,
		)
		if callErr != nil {
			return callErr
		}
		return callErr
	})
	return result, callErr
}

// OpenBlob calls android.app.blob.BlobStoreManager.openBlob.
func (m *StoreManager) OpenBlob(arg0 *jni.Object) (*jni.Object, error) {
	var result *jni.Object
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midStoreManagerOpenBlob == nil {
			callErr = fmt.Errorf("android.app.blob.BlobStoreManager.openBlob is not available on this device")
			return callErr
		}

		result, callErr = env.CallObjectMethod(
			m.Obj,
			midStoreManagerOpenBlob, jni.ObjectValue(arg0),
		)
		if callErr != nil {
			return callErr
		}
		// Convert the JNI local reference to a global reference so the
		// returned object remains valid outside this vm.Do scope.
		if result != nil {
			localRef := result
			result = env.NewGlobalRef(localRef)
			env.DeleteLocalRef(localRef)
		}
		return callErr
	})
	return result, callErr
}

// OpenSession calls android.app.blob.BlobStoreManager.openSession.
func (m *StoreManager) OpenSession(arg0 int64) (*jni.Object, error) {
	var result *jni.Object
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midStoreManagerOpenSession == nil {
			callErr = fmt.Errorf("android.app.blob.BlobStoreManager.openSession is not available on this device")
			return callErr
		}

		result, callErr = env.CallObjectMethod(
			m.Obj,
			midStoreManagerOpenSession, jni.LongValue(arg0),
		)
		if callErr != nil {
			return callErr
		}
		// Convert the JNI local reference to a global reference so the
		// returned object remains valid outside this vm.Do scope.
		if result != nil {
			localRef := result
			result = env.NewGlobalRef(localRef)
			env.DeleteLocalRef(localRef)
		}
		return callErr
	})
	return result, callErr
}

// ReleaseLease calls android.app.blob.BlobStoreManager.releaseLease.
func (m *StoreManager) ReleaseLease(arg0 *jni.Object) error {

	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midStoreManagerReleaseLease == nil {
			callErr = fmt.Errorf("android.app.blob.BlobStoreManager.releaseLease is not available on this device")
			return callErr
		}

		callErr = env.CallVoidMethod(
			m.Obj,
			midStoreManagerReleaseLease, jni.ObjectValue(arg0),
		)
		return callErr
	})
	return callErr
}

// ToString calls android.app.blob.BlobStoreManager.toString.
func (m *StoreManager) ToString() (string, error) {
	var result string
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midStoreManagerToString == nil {
			callErr = fmt.Errorf("android.app.blob.BlobStoreManager.toString is not available on this device")
			return callErr
		}
		var resultObj *jni.Object
		resultObj, callErr = env.CallObjectMethod(
			m.Obj,
			midStoreManagerToString,
		)
		if callErr != nil {
			return callErr
		}
		result = env.GoString((*jni.String)(unsafe.Pointer(resultObj)))
		return callErr
	})
	return result, callErr
}
