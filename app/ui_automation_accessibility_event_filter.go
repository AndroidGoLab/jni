// Code generated by javagen. DO NOT EDIT.

package app

import (
	"fmt"
	"unsafe"

	"github.com/AndroidGoLab/jni"
)

// Ensure imports are used.
var (
	_ = fmt.Errorf
	_ = unsafe.Pointer(nil)
)

// UiAutomationAccessibilityEventFilter wraps android.app.UiAutomation$AccessibilityEventFilter.
type UiAutomationAccessibilityEventFilter struct {
	VM  *jni.VM
	Obj *jni.GlobalRef
}

// Accept calls android.app.UiAutomation$AccessibilityEventFilter.accept.
func (m *UiAutomationAccessibilityEventFilter) Accept(arg0 *jni.Object) (bool, error) {
	var result bool
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midUiAutomationAccessibilityEventFilterAccept == nil {
			callErr = fmt.Errorf("android.app.UiAutomation$AccessibilityEventFilter.accept is not available on this device")
			return callErr
		}

		var resultRaw uint8
		resultRaw, callErr = env.CallBooleanMethod(
			m.Obj,
			midUiAutomationAccessibilityEventFilterAccept, jni.ObjectValue(arg0),
		)
		if callErr != nil {
			return callErr
		}
		result = resultRaw != 0
		return callErr
	})
	return result, callErr
}

// ToString calls android.app.UiAutomation$AccessibilityEventFilter.toString.
func (m *UiAutomationAccessibilityEventFilter) ToString() (string, error) {
	var result string
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midUiAutomationAccessibilityEventFilterToString == nil {
			callErr = fmt.Errorf("android.app.UiAutomation$AccessibilityEventFilter.toString is not available on this device")
			return callErr
		}
		var resultObj *jni.Object
		resultObj, callErr = env.CallObjectMethod(
			m.Obj,
			midUiAutomationAccessibilityEventFilterToString,
		)
		if callErr != nil {
			return callErr
		}
		result = env.GoString((*jni.String)(unsafe.Pointer(resultObj)))
		return callErr
	})
	return result, callErr
}
