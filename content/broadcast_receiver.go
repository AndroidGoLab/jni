...
// ToString calls android.content.BroadcastReceiver.toString.
func (m *BroadcastReceiver) ToString() (string, error) {
	var result string
	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
		...
		if midBroadcastReceiverToString == nil {
			callErr = fmt.Errorf("android.content.BroadcastReceiver.toString is not available on this device")
			return callErr
		}
		var resultObj *jni.Object
		resultObj, callErr = env.CallObjectMethod(
			m.Obj,
			midBroadcastReceiverToString,
		)
		if callErr != nil {
			return callErr
		}
