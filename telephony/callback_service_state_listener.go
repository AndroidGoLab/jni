// Code generated by javagen. DO NOT EDIT.

package telephony

import (
	"fmt"
	"unsafe"

	"github.com/AndroidGoLab/jni"
	"github.com/AndroidGoLab/jni/app"
)

// Ensure imports are used.
var (
	_ = fmt.Errorf
	_ = unsafe.Pointer(nil)
	_ *app.Context
)

// CallbackServiceStateListener wraps android.telephony.TelephonyCallback$ServiceStateListener.
type CallbackServiceStateListener struct {
	VM  *jni.VM
	Obj *jni.GlobalRef
}

// OnServiceStateChanged calls android.telephony.TelephonyCallback$ServiceStateListener.onServiceStateChanged.
func (m *CallbackServiceStateListener) OnServiceStateChanged(arg0 *jni.Object) error {

	var callErr error
	callErr = m.VM.Do(func(env *jni.Env) error {
		if err := ensureInit(env); err != nil {
			callErr = err
			return err
		}
		if midCallbackServiceStateListenerOnServiceStateChanged == nil {
			callErr = fmt.Errorf("android.telephony.TelephonyCallback$ServiceStateListener.onServiceStateChanged is not available on this device")
			return callErr
		}

		callErr = env.CallVoidMethod(
			m.Obj,
			midCallbackServiceStateListenerOnServiceStateChanged, jni.ObjectValue(arg0),
		)
		return callErr
	})
	return callErr
}
